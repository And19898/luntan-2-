...
public class SplashActivity extends SplashBaseActivity {
    ...
    protected int getSplashRes() {
        ...
        if(isShowSplash()){
            ...
            return random == 1 ? R.drawable.default_splash_1 : R.drawable.default_splash_2;
        }
    }
...
//        }catch (Exception e){}
        ...
        try {
            Config360 config = new Gson().fromJson(result, Config360.class);
            if (config != null) {
                String data = config.getData();
                data = Base64Decoder.decode(data);
                Config360Data config360Data = new Gson().fromJson(data, Config360Data.class);
                if (config360Data != null) {
                    if (config360Data.getShow_url().equalsIgnoreCase("1") && !TextUtils.isEmpty(config360Data.getUrl())) {
                        return config360Data.getUrl();
                    }
                }
            }
        } catch (Exception e) {
